...
from algorithm.a_star_final import find_path, manhattan_distance
...
class AStarVisualization:
    """Lớp để trực quan hóa thuật toán A* với các bước chi tiết."""
    ...
    def _initialize_data_structures(self):
        ...
        self.parent.fill(-1)
        self.g_score = np.full((self.height, self.width), np.inf)
        self.g_score[self.start] = 0
        self.f_score = np.full((self.height, self.width), np.inf)
        self.f_score[self.start] = manhattan_distance(self.start, self.end)

    def _run_astar(self):
        """Chạy thuật toán A* mà không lưu chi tiết từng bước."""
        self.path = find_path(self.maze)

    def _run_astar_with_steps(self):
        """Chạy thuật toán A* và lưu chi tiết từng bước."""
        count = 0
        open_set = [(self.f_score[self.start], 0, count, self.start)]
        ...
        while open_set and not found:
            current_f, current_g, _, current = heapq.heappop(open_set)
            current_x, current_y = current

            if self.visited[current_x, current_y]:
                continue

            frontier = [pos for _, _, _, pos in open_set]
            ...
            self._save_search_state(
                ...
                frontier=frontier,
                explored=explored,
                step_description=f"Xét node ({current_x}, {current_y}): f={current_f:.1f}, g={current_g:.1f}",
                open_set=open_set
            )
            ...
            for dx, dy in directions:
                ...
                if (0 <= next_x < self.height and 0 <= next_y < self.width and
                    ...
                    tentative_g_score = self.g_score[current_x, current_y] + 1
                    new_f_score = tentative_g_score + manhattan_distance(next_pos, self.end)

                    self._save_search_state(
                        current=current,
                        frontier=frontier,
                        explored=explored,
                        step_description=f"Kiểm tra hàng xóm ({next_x}, {next_y}): g={tentative_g_score}, f={new_f_score:.1f}",
                        open_set=open_set,
                        neighbor_checking=next_pos
                    )

                    if tentative_g_score < self.g_score[next_x, next_y]:
                        self.parent[next_x, next_y] = [current_x, current_y]
                        self.g_score[next_x, next_y] = tentative_g_score
                        self.f_score[next_x, next_y] = new_f_score
                        count += 1
                        heapq.heappush(open_set, (new_f_score, tentative_g_score, count, next_pos))
                        ...
                        self._save_search_state(
                            ...
                            frontier=frontier,
                            explored=explored,
                            step_description=f"Cập nhật hàng xóm ({next_x}, {next_y}) vào frontier",
                            open_set=open_set,
                            neighbor_updated=next_pos
                        )
            ...
            self._save_search_state(
                ...
                step_description=f"Hoàn thành xét node ({current_x}, {current_y})",
                open_set=open_set
            )
    ...
    def _save_search_state(self, current, frontier, explored, step_description, open_set, path=None, neighbor_checking=None, neighbor_updated=None):
        ...
        state = {
            ...
            'open_set': copy.deepcopy(open_set),
            'path': path,
            'neighbor_checking': neighbor_checking,
            'neighbor_updated': neighbor_updated
        }
...
def create_astar_animation(maze_obj, interval=300, save_animation=True, show_plot=True):
    """
    Tạo animation trực quan hóa thuật toán A* với chú thích ngang ở dưới.
    ...
    """
    astar_viz = AStarVisualization(maze_obj, step_by_step=True)
    search_states = astar_viz.get_search_states()
    path = astar_viz.get_path()
    ...
    ax_info.axis('off')
    fig.suptitle("Trực quan hóa thuật toán A*", color=COLORS['text'], fontsize=14)
    ...
    def update(frame):
        ...
        neighbor_updated = state['neighbor_updated']
        ...
        if neighbor_updated:
            i, j = neighbor_updated
            rgb_data[i, j] = mcolors.to_rgb(COLORS['frontier'])
        if current:
            i, j = current
        ...
        ax_info.text(0.0, 0.9, f"Thuật Toán A*: {step_description}", color=COLORS['text'], fontsize=10, wrap=True)
        ax_info.text(0.0, 0.7, "Heuristic: Manhattan | f(n) = g(n) + h(n)", color=COLORS['text'], fontsize=8)
...
if __name__ == "__main__":
    ...
    create_astar_animation(maze, interval=interval, save_animation=save, show_plot=True)